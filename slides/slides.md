...
- **4일 빌드 + 1일 쿨다운 = 한 주가 한 심박** — 월 베팅, 목 검증 게이트, 금 쿨다운
- 사이클은 작업 시간이 아니라 **재베팅 리듬** — 배포는 슬라이스 단위로 수시
- 변형: 상향 8일+2일(2주, 엔터프라이즈) / 하향 2일+1일(검증 자산 성숙 팀·솔로)
...
| Shape Up | VDLC 주간 사이클 |
|---|---|
| Circuit breaker: 미완료는 취소 | **컨텍스트 브레이커**: shaping 반송, Day 1 조기 발동 |
| Betting: 한 팀을 6주 묶는 헌신 | **병렬 베팅**: 옵션 매수, 2~3개 접근 후 승자 선택 |
| Hill chart: 진행 보고 도구 | **위임 가능성 지도**: uphill=인간 판단, downhill=에이전트 위임 |
| QA is for the edges | **검증 게이트**: Day 4 전체가 검증, 코딩 금지 |
...
- 전문(매니페스토), 단계별 플레이북, 주간 사이클, 도입 가이드, 릴리즈 노트
