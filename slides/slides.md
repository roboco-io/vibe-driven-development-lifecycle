...
<div class="text-sm opacity-50 mt-4">v1.2</div>
...
Shape Up의 바이브 코딩 재해석

---

# 왜 6주가 아니라 6일인가

- Shape Up의 6주는 **인간이 코드를 작성한다**는 가정 위의 계산
- 구현이 분·시간 단위로 끝나면 사이클 길이의 변수는 shaping·검증에 드는 인간의 시간
- **6일 빌드 + 2일 쿨다운** = 약 1.5주가 한 심박(heartbeat) — 기본값이지 하한 아님
- 사이클은 작업 시간이 아니라 **재베팅 리듬** — 배포는 슬라이스 단위로 수시
- 변형: 상향 10일+3일(엔터프라이즈) / 하향 2일+1일(검증 자산 성숙 팀·솔로)
- 하한은 수치가 아니라 캘리브레이션 — 브레이커 발동률·게이트 1회 통과율로 조정

---

# 의미가 반전된 장치들

| Shape Up | VDLC 6일 사이클 |
|---|---|
| Circuit breaker: 미완료는 취소 | **컨텍스트 브레이커**: shaping 반송, Day 2 조기 발동 |
| Betting: 한 팀을 6주 묶는 헌신 | **병렬 베팅**: 옵션 매수, 2~3개 접근 후 승자 선택 |
| Hill chart: 진행 보고 도구 | **위임 가능성 지도**: uphill=인간 판단, downhill=에이전트 위임 |
| QA is for the edges | **검증 게이트**: Day 6 전체가 검증, 코딩 금지 |

---

# 프로토타입 주도 Shaping

- Shape Up의 shaping = 시니어의 암묵지 — 프로토타입이 비쌌던 시대의 산물
- 부등호가 뒤집혔다: **"만들어 보고 판단" < "논의해서 판단"의 비용**
- 구현 가속은 절반의 활용 — 나머지 절반은 **의사결정 속도의 최적화**
- **수렴형**: 한 접근을 고속 반복 — 방향의 불확실성
- **발산형**: 2~5개 병렬 생성 후 인간이 선택 — UI·취향 판단, 큐레이션이 인간 역량

---

# 결정기록 — 스펙과 프로토타입을 묶다

- 프로토타입은 throwaway — 승자도 프로덕션 승격 금지, 빌드 사이클에서 재생성
- 폐기 전 에이전트가 **결정기록** 자동 생성: 시도한 접근 / 승자와 근거 / 탈락 사유
- 핏치가 결정기록을 링크 — "왜 이 형태인가"가 컨텍스트 자산으로 보존
- 쿨다운의 제1과업은 **재생성 검증** — 문서만으로 재생성 실패 시 숨은 의도를 역이전
...
- 전문(매니페스토), 단계별 플레이북, 6일 사이클, 도입 가이드, 릴리즈 노트
