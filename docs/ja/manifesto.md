...
チーム単位の実行リズムとしては、Basecamp Shape Upの6週間サイクルをVDLCの前提のうえで再解釈した**6日サイクル**を推奨する。実装が分・時間単位で終わる時代に、サイクルの長さを決める変数はビルド時間ではなく、shaping(意図定義)と検証にかかる人間の時間であるため、週を日に置き換えて6日ビルド + 2日クールダウンを一つの心拍として回す。shapingは文書活動ではなく、プロトタイプラウンドを内蔵した実験活動になる — 作ってみて判断することが、議論して判断することより安くなったからだ。プロトタイプラウンドごとに自動生成される決定記録が、「なぜこの形に決まったのか」の文脈をコンテキスト資産として保存する。

サイクル構造、コンテキストブレーカー、並列ベット、ピッチテンプレート、再生成検証の全体的な定義は[6日サイクル](/ja/cycle)で扱う。
