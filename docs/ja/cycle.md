...
| 時間単位の変更 | 6週間サイクル、2週間クールダウン | 週→日単位へ圧縮(4日ビルド + 1日クールダウン、一週間が一つの心拍) |
| 意味の反転 | Circuit breaker, Betting, Hill chart, QA is for the edges, 「Shapingはシニアの暗黙知」 | 以下で詳述 |

## サイクル構造

週を日に置き換えつつ、心拍はカレンダー週に揃える。**4営業日ビルド + 1日クールダウン = 一週間が一つの心拍(heartbeat)** だ。月曜の朝にベットし、金曜にクールダウンで一週間を締める。比率はShape Upの3:1から4:1へ移した — クールダウンの作業(再生成検証、負債の逆移転)もエージェントが実行するため、一日で十分だ。

```mermaid
flowchart TB
    S["<b>Shapingトラック(常時並列)</b><br/>問題定義 → Breadboard → プロトタイプラウンド → ピッチ"]
    B["<b>ビルドサイクル(4日・月~木)</b><br/>Day 0 ベット → Day 1 垂直スライス → Day 2-3 委任 → Day 4 検証ゲート"]
    C["<b>クールダウン(1日・金)</b><br/>再生成検証 → 次のベッティングテーブル"]
    S -->|"ピッチ確定"| B
    B -->|"ゲート通過"| C
    B -. "収束失敗時<br/>コンテキストブレーカー" .-> S
```
...
| Day | 活動 | 完了条件 |
|---|---|---|
| Day 0(月朝) | ベッティングテーブル。ピッチ検討、並列ベットの配分、勝者選定基準の事前文書化 | 各ベットに二重予算(人間の注意力 + コンピュート)を明示済み |
| Day 1(月) | Get one piece done. 垂直スライス一つをデプロイ可能な状態へ統合 | 実環境で動作するend-to-endスライス1個 |
| Day 2–3(火・水) | スコープマップに基づく委任。downhillスコープはエージェントへ完全委任、uphillスコープは人間の判断 | すべてのmust-haveスコープがdownhillへ突入 |
| Day 4(木) | 検証ゲート。コーディング禁止、検証専用 | Done = Deployed 判定 |
| Day 5(金) | クールダウン。再生成検証の儀式、負債の逆移転、ad-hoc作業、次のベッティングテーブルの準備 | コンテキスト文書のみで再生成成功、または負債の逆移転完了 |

### サイクル長のキャリブレーション

一週間(4日 + 1日)はデフォルト値であって下限ではない。サイクルは作業時間の見積もりではなく**再ベットのリズム**だ。デプロイはサイクルの中でスライス単位に随時起こるため(Day 1からDone = Deployed)、サイクルを縮めて得られるのはより速いデプロイではなく、より頻繁な意思決定だ。したがって適正な長さは3つの変数の関数だ。
...
| バリアント | 構成 | 適用対象 |
|---|---|---|
| デフォルト | 4日ビルド + 1日クールダウン(一週間) | 成熟度が実践(Practicing)水準のチーム |
| 上方 | 8日ビルド + 2日クールダウン(2週間) | shaping・合意にリーダーシップの承認が必要なエンタープライズ |
...
発動条件(一つでも満たせば即時発動、Day 4まで待たない)。

1. Day 1終了時点でデプロイ可能な垂直スライスがない
...
Shape UpのQAはエッジケースのみを扱った — 人間の実装者がメインフローをすでに検証したという前提だった。VDLCにはその前提がない。検証が明示的なステージへ昇格し、**Day 4全体が検証に割り当てられる**。Day 4には新規コーディングを禁止する(検証中に発見された欠陥の修正のみ許可)。
...
- バッチサイズ: Small(半日〜1日) | Big(ビルド4日全体)
...
4. **失敗**: コードに文書化されていない意図が隠れているという意味だ。diffを分析して隠れた意図をコンテキスト文書へ**逆移転**する。逆移転の完了前には次のサイクルのベットを禁止する。

技術負債ではなく**コンテキスト負債**(コードにはあるが意図文書にはない決定)の清算が、クールダウンの第一の課業だ。

## 用語対応表

| Shape Up | VDLC 週間サイクル | 変更の要旨 |
|---|---|---|
| Cycle(6週間) | ビルドサイクル(4日) | 週→日、一週間が一つの心拍。上方バリアント8日、下方バリアント2日 |
| Cool-down(2週間) | クールダウン(1日) | 再生成検証が第一の課業 |
...
| Circuit breaker | コンテキストブレーカー | キャンセル → shaping差し戻し。Day 1の早期発動 |
| Hill chart | 委任可能性マップ | 報告ツール → 人間-エージェントの業務分配ツール |
| Scope hammering | コンテキストハンマリング | コード削除ではなく意図文書の修正後に再生成 |
| QA is for the edges | 検証ゲート(Day 4) | 補助活動 → 明示的ステージへ昇格 |
