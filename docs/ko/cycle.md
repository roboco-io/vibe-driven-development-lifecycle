...
| 시간 단위 변경 | 6주 사이클, 2주 쿨다운 | 주 → 일 단위로 압축 (6일 빌드 + 2일 쿨다운) |
...
| 쿨다운 1일차 | 재생성 검증 의식 | 컨텍스트 문서만으로 재생성 성공 또는 부채 역이전 완료 |
| 쿨다운 2일차 | ad-hoc 작업, 버그 수정, 다음 베팅 테이블 준비 | — |

### 사이클 길이 캘리브레이션

6일은 기본값이지 하한이 아니다. 사이클은 작업 시간의 추정치가 아니라 **재베팅 리듬**이다. 배포는 사이클 안에서 슬라이스 단위로 수시로 일어나므로(Day 1–2부터 Done = Deployed), 사이클을 줄여 얻는 것은 더 빠른 배포가 아니라 더 잦은 의사결정이다. 따라서 적정 길이는 세 변수의 함수다.
...
| 변형 | 구성 | 적용 대상 |
|---|---|---|
| 기본 | 6일 빌드 + 2일 쿨다운 | 성숙도 실천(Practicing) 수준의 팀 |
| 상향 | 10일 빌드 + 3일 쿨다운 | shaping·합의에 리더십 승인이 필요한 엔터프라이즈 |
...
Shape Up의 QA는 엣지케이스만 다뤘다 — 인간 구현자가 메인 플로우를 이미 검증했다는 전제였다. VDLC에는 그 전제가 없다. 검증이 명시적 단계로 승격되며 **Day 6 전체가 검증에 배정**된다. Day 6에는 신규 코딩을 금지한다(검증 중 발견된 결함의 수정만 허용).
...
|---|---|---|
| Cycle (6주) | 빌드 사이클 (6일) | 주→일. 상향 변형 10일, 하향 변형 2일 |
