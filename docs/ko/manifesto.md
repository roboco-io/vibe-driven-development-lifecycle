...
팀 단위의 실행 리듬으로는 Basecamp Shape Up의 6주 사이클을 VDLC 전제 위에서 재해석한 **6일 사이클**을 권장한다. 구현이 분·시간 단위로 끝나는 시대에 사이클 길이를 결정하는 변수는 빌드 시간이 아니라 shaping(의도 정의)과 검증에 드는 인간의 시간이므로, 주를 일로 치환해 6일 빌드 + 2일 쿨다운을 한 심박으로 돈다. shaping은 문서 활동이 아니라 프로토타입 라운드를 내장한 실험 활동이 된다 — 만들어 보고 판단하는 것이 논의해서 판단하는 것보다 싸졌기 때문이다. 프로토타입 라운드마다 자동 생성되는 결정기록이 "왜 이 형태로 정해졌는가"의 맥락을 컨텍스트 자산으로 보존한다.

사이클 구조, 컨텍스트 브레이커, 병렬 베팅, 핏치 템플릿, 재생성 검증의 전체 정의는 [6일 사이클](/ko/cycle)에서 다룬다.
