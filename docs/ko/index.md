...
hero:
  ...
  actions:
    ...
    - theme: alt
      text: 실무 가이드
      link: /ko/guide/intent
    - theme: alt
      text: 도입 로드맵
      link: /ko/adoption/roadmap
features:
  - title: 의도가 소스다
...
- [매니페스토](/ko/manifesto) — VDLC의 정의, 배경, 여섯 가지 원칙, 기존 방법론과의 관계를 담은 원문
- [실무 가이드](/ko/guide/intent) — 여섯 단계 각각을 실행하는 방법을 다루는 플레이북
- [주간 사이클](/ko/cycle) — Shape Up을 바이브 코딩 전제로 재해석한 팀 운영 리듬. 프로토타입 주도 shaping과 결정기록 포함
- [템플릿](/ko/templates/) — 의도 문서, PR-FAQ, 리스크 매트릭스 등 반복 사용하는 문서 양식
- [도입](/ko/adoption/) — 성숙도 모델, 도입 로드맵, 측정 지표로 조직 도입 경로를 설계하는 문서
