...
[4단계 — 검증](/ko/guide/verify)에서 에이전트 산출물을 사람 또는 교차 리뷰 에이전트가 검토할 때 쓴다. 코드 리뷰를 "잘 짰는지" 위주로만 보면 에이전트 특유의 실수 — 의도와 어긋난 구현, 실제로는 아무것도 검증하지 않는 테스트, 불필요하게 방어적인 코드 — 를 놓치기 쉽다. 이 체크리스트는 그런 실수를 항목으로 못 박아 리뷰어가 매번 같은 것을 놓치지 않게 한다.
