...
2. **리스크 등급을 매긴다.** 이 산출물이 실패했을 때 비용이 얼마나 큰지 판단한다. 결제·인증·개인정보처럼 실패 비용이 큰 영역인지, 내부 도구·프로토타입처럼 가벼운 영역인지 분류한다. 기준은 [리스크 매트릭스](/ko/templates/risk-matrix)를 따른다.
...
- 리스크 등급이 부여된 검증 계획 — [리스크 매트릭스](/ko/templates/risk-matrix)
- 리뷰 기록 — [리뷰 체크리스트](/ko/templates/review-checklist)
